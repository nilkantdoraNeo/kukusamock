...
const textPath = 'C:/Users/Client/Desktop/Quiz/hehe.txt';
const envPath = 'C:/Users/Client/Desktop/Quiz/backend/.env';

const START_MARKER = 'Surgery, Gynaecology & Obstetrics';
const EXAM_SLUG = 'cms-2025-paper-2';
...
function applyFixups(questions) {
  const byNumber = new Map(questions.map(q => [q.number, q]));

  const q94 = byNumber.get(94);
  if (q94) {
    q94.options = {
      a: 'cT1 N1 Mx',
      b: 'cT2 N1 Mx',
  ...
  const q108 = byNumber.get(108);
  if (q108) {
    q108.options.d = q108.options.d || 'PCO2';
  }

  const q109 = byNumber.get(109);
  if (q109) {
    q109.options.a = 'Ileum';
  }
...
async function main() {
  ...
  questions = applyFixups(questions);
  ...
  if (existing?.id) {
    ...
    await supabase
      .from('exams')
      .update({
        name: 'CMS 2025 Paper 2',
        description: 'UPSC CMS 2025 Surgery, Gynaecology & Obstetrics',
        is_active: true,
        duration_seconds: 7200,
        question_limit: merged.length,
        marks_per_question: 2.083,
        negative_mark_ratio: 0.333333
      })
      .eq('id', examId);

    // Remove existing questions for this exam to avoid duplicates
    const { error: delErr } = await supabase.from('quizzes').delete().eq('exam_id', examId);
